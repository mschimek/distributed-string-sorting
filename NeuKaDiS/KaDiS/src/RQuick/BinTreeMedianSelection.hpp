...
std::vector<unsigned char> select(Iterator begin,
    Iterator end, size_t n, Comp&& comp, MPI_Datatype mpi_type,
...
void selectMedians(std::vector<T>& recv_v, std::vector<T>& tmp_v,
    ...
    tmp_v.resize(recv_v.size() + vs.size()); // recv_v and vs contain 0 terminated raw strings
    ...
    std::vector<String> tmp_v_strings(recv_v_container.size() + vs_container.size());
    
    std::merge(recv_v_container.getStrings().begin(), recv_v_container.getStrings().end(), vs_container.getStrings().begin(), vs_container.getStrings().end(),
        tmp_v_strings.begin(), std::forward<Comp>(comp));

    {
    uint64_t curPos = 0;
    for (const auto& str : tmp_v_strings) {
      std::copy(str.string, str.string + str.length + 1, tmp_v.data() + curPos);
      curPos += str.length + 1;
    }
    }
    auto copyTmp = tmp_v;
    StringContainer tmpContainer(std::move(copyTmp));
            //std::cout << "iteration: " << globalIteration << " on rank: " << globalRank << std::endl;
    //tmpContainer.make_string_set().print("iteration" + std::to_string(globalIteration) + " on rank: " + std::to_string(globalRank));
...
std::vector<unsigned char> select(Iterator begin,
    ...
    for (size_t it = 0; it != iterations; ++it) {
        ...
        RBC::Recv(recv_v.data(), count, mpi_type, source, tag, comm,
            MPI_STATUS_IGNORE);
    ...
    if (myrank == 0) {
        ...
        auto medianIndex = _internal::selectMedian(container.getStrings(), async_gen, bit_gen);

        auto requestedRawString = container.getRawString(medianIndex);
        int32_t size = requestedRawString.size();
        RBC::Bcast(&size, 1, MPI_INT, 0, comm);
        RBC::Bcast(requestedRawString.data(), size, MPI_BYTE, 0, comm);
        return requestedRawString;
    }
    else {
        int target = myrank - (1 << tailing_zeros);
        assert(v.size() <= n);
        RBC::Send(v.data(), v.size(), MPI_BYTE, target, tag, comm);

        int32_t medianSize;
        RBC::Bcast(&medianSize, 1, MPI_INT, 0, comm);
        std::vector<unsigned char> median(medianSize);
        RBC::Bcast(median.data(), medianSize, MPI_CHAR, 0, comm);
        return median;
    }
}
} // namespace BinTreeMedianSelection
