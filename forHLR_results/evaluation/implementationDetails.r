...
lineplot <- function(datasets, operation_, type_ = "maxTime", title = " ", blacklist = c()) {
  ...
  for (i in c(1:length(datasets))) {
    ...
    for (k in c(1:numberFilters)){
      ...
      #print(localSets[[counter]])
      localSets[[counter]] <- mutate(localSets[[counter]], name = df[k, 1])
      localSets[[counter]] <- select(localSets[[counter]], numberProcessors, dToNRatio,  operation, type, name, iteration, value, size)
      if (k == 1 && i == 1){
        set <- localSets[[counter]]
      }
      else {
...
l <- lineplot(c(1:length(data)), "all_to_all_strings", "maxTime", title)
...
#plot_grid(top, legend, nrow = 2, rel_heights = c(1, 0.2))
