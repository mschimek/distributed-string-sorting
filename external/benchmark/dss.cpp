...
std::int32_t main(std::int32_t argc, char const *argv[]) {
  ...
  for (const auto& algorithm : algorithm_list) {
    if (algorithm->category() == dsss::algorithm_category::DISTRIBUTED) {
      ...
      std::string string_output = "PE: " + std::to_string(env.rank())  + " strings_to_sort.size() " + 
        std::to_string(local_size);
      for (auto i = 0; i < strings_to_sort.size(); ++i)
      {
        string_output +=  " " + std::string(reinterpret_cast<char *>(strings_to_sort[i]));
      }
      ...
      std::cout << "rank: " << env.rank() << " check: " << check << std::endl;
      env.barrier();
      if (check) {
        std::string output = "PE: " +  std::to_string(env.rank()) + " ";
        for (size_t i = 0; i < strings_to_sort.size(); ++i)
          output += std::string(reinterpret_cast<char*>(strings_to_sort[i])) + " ";
        ...
        std::cout << strings_to_sort.size() << std::endl;
        auto smaller_string =
