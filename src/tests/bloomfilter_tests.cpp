...
class BloomfilterTester : HashPolicy {
    ...
    struct HashesEOSHashes {
        ...
        std::vector<size_t>
            eosHashes; // End Of String hashes, i.e. hashes from strings that
...
public:
    BloomfilterTester(size_t bloomFilterSize)
        : bloomFilterSize(bloomFilterSize) {}

    DuplicateUniqueHashes filter(StringSet ss,
        ...
        // -> can contain duplicates if eosHash and globalHash has same hash
        // value but this is no problem
...
private:
    ...
    HashesEOSHashes generateHashes(StringSet ss,
        const std::vector<size_t>& candidates, const size_t depth) {
        ...
        for (const size_t curCandidate : candidates) {
            ...
            if (depth > length) {
                const size_t hash = HashPolicy::hash(
                    ss.get_chars(curString, 0), length, bloomFilterSize);
    ...
    DuplicateUniqueHashes getDuplicateUniqueHashes(
        std::vector<size_t>& hashes) {
        ...
        // special cases
        if (hashes.empty()) return hashContainer;
        if (hashes.size() == 1) {
        ...
        for (size_t i = 0; i + 1 < hashes.size();) {
            ...
            while (indexFirstDiffHash < hashes.size() &&
                   curHash == hashes[indexFirstDiffHash])
                ++indexFirstDiffHash; // advance til hashes are different or end
                                      // of vector
...
std::vector<size_t> getDifference(
    std::vector<size_t> minuendSet, std::vector<size_t> subtrahendSet) {
...
struct CharacterBasesHash {
    static inline size_t hash(
        const unsigned char* str, size_t maxDepth, size_t m) {
        size_t hash = 5381;
        size_t c = 0, i = 0;
...
std::vector<size_t> getHashValues(StringSet ss,
    const std::vector<size_t>& indicesToHash, const size_t depth,
...
void testResult(std::vector<size_t> globalDuplicateHashes,
    ...
    const bool foundAllUniqueHashes = std::all_of(hashesUniqueElems.begin(),
        hashesUniqueElems.end(), [&](const size_t hash) {
            return std::binary_search(
                globalUniqueHashes.begin(), globalUniqueHashes.end(), hash);
        });

    // verify that all duplicate hashes found by the bloomfilter are also found
    // by the tester
    std::sort(globalDuplicateHashes.begin(), globalDuplicateHashes.end());
    const bool foundAllDuplicateHashes = std::all_of(hashesDuplicates.begin(),
        hashesDuplicates.end(), [&](const size_t hash) {
            return std::binary_search(globalDuplicateHashes.begin(),
                globalDuplicateHashes.end(), hash);
        });
...
void bloomfilter_test(
    const size_t size, const size_t stringLength, const double dToNRatio) {
    ...
    BloomfilterTester<HashPolicy, StringSet> bftest(
    ...
    duplicates.reserve(
        actualSize); // all indices are duplicates at the beginning
    // std::generate_n(std::back_inserter(duplicates), actualSize, []() {static
    // size_t i = 0; return i++;});

    for (size_t depth = startDepth; depth < std::numeric_limits<size_t>::max();
         depth *= 2) {
        ...
        duplicates =
            bloomFilter.filter(localStringPtr, depth, candidates, results);
        auto duplicatesUniques = bftest.filter(ss, candidates, depth);

        std::vector<size_t>& globalDuplicateHashes =
            duplicatesUniques.duplicates;
        std::vector<size_t>& globalUniqueHashes = duplicatesUniques.uniques;

        std::vector<size_t> hashesDuplicates =
            getHashValues<StringSet, HashPolicy>(
                ss, duplicates, depth, bloomFilter.bloomFilterSize);
        std::vector<size_t> indicesUniqueElems =
            getDifference(candidates, duplicates);
        std::vector<size_t> hashesUniqueElems =
            getHashValues<StringSet, HashPolicy>(
                ss, indicesUniqueElems, depth, bloomFilter.bloomFilterSize);

        // basic tests
        tlx_die_unless(
            depth != startDepth ||
            candidates.size() == actualSize); // depth == startDepth -> all
                                              // indices are candidates
        tlx_die_unless(
            indicesUniqueElems.size() + duplicates.size() == candidates.size());

        tlx_die_unless(
            indicesUniqueElems.size() == 0 ||
            hashesUniqueElems.size() >
                0); // ther are some uniques -> there are some unique Hashes
        tlx_die_unless(indicesUniqueElems.size() <= hashesUniqueElems.size());

        tlx_die_unless(duplicates.size() == 0 ||
                       hashesDuplicates.size() >
                           0); // ther are some dup -> there are some unique dup
        tlx_die_unless(duplicates.size() <= hashesDuplicates.size());

        // complete test: verify that all unique hashes found by the bloomfilter
        // are also found by the tester
        testResult(globalDuplicateHashes, globalUniqueHashes, hashesDuplicates,
            hashesUniqueElems);
