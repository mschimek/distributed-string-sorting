...
public:
    ...
    static CompressedIntegersCounts writeRanges(CountIterator beginCounts,
        CountIterator endCounts, InputIterator dataInputBegin) {
    ...
    static uint64_t read(
        InputIterator input, OutputIterator begin, OutputIterator end) {
    ...
    template <typename CountIterator, typename InputIterator>
    static std::vector<uint64_t> readRanges(CountIterator beginCounts,
        CountIterator endCounts, InputIterator dataInputBegin) {
        const uint64_t sumOfRecvLcpValues =
            std::accumulate(beginCounts, endCounts, 0ull);
        std::vector<uint64_t> output(sumOfRecvLcpValues);
        uint64_t curNumWrittenLcps = 0u;
        uint64_t curNumConsumedBytes = 0u;
        for (CountIterator it = beginCounts; it != endCounts; ++it) {
            curNumConsumedBytes += read(dataInputBegin + curNumConsumedBytes,
...
public:
    ...
    std::pair<unsigned char*, size_t> write(
        unsigned char* buffer, const StringSet ss) const {
...
public:
    ...
    template <typename StringSet>
    std::pair<unsigned char*, size_t> write(
        unsigned char* buffer, const StringSet ss, const size_t* lcps) const {
...
public:
    ...
    std::pair<unsigned char*, size_t> write(unsigned char* buffer,
        const StringSet ss, const size_t* lcps,
        const size_t* distinguishingPrefixes) const {
...
public:
    ...
    size_t computeNumberOfSendBytes(
        size_t charsToSend, size_t numbersToSend) const {
        return charsToSend + sizeof(size_t) * numbersToSend +
               2 * sizeof(size_t);
    }
    ...
    size_t computeNumberOfSendBytes(const std::vector<size_t>& charsToSend,
        ...
        for (size_t i = 0; i < charsToSend.size(); ++i) {
            numberOfSendBytes +=
                computeNumberOfSendBytes(charsToSend[i], numbersToSend[i]);
        }
    ...
    unsigned char* write(unsigned char* buffer,
        const unsigned char* charsToWrite, size_t numChars,
        const size_t* numbersToWrite, size_t numNumbers) const {
    ...
    void read_(unsigned char* buffer, std::vector<unsigned char>& charsToRead,
        ...
        for (size_t i = 0; i < numNumbersToRead; ++i) {
            size_t tmp;
            std::memcpy(&tmp, buffer + numCharsToRead + i * sizeof(size_t),
                sizeof(size_t));
    ...
    std::pair<std::vector<unsigned char>, std::vector<size_t>> read(
        ...
        while (curPos < size) {
            ...
            for (size_t i = 0; i < numNumbersToRead; ++i) {
                size_t tmp;
                std::memcpy(
                    &tmp, buffer + curPos + i * sizeof(size_t), sizeof(size_t));
                numbersToRead.emplace_back(tmp);
            }
...
public:
    ...
    // start work here
    template <typename StringSet>
    unsigned char* write(unsigned char* buffer, const StringSet ss,
...
public:
    ...
    std::pair<std::vector<unsigned char>, std::vector<size_t>> read(
        ...
        while (
            curPos <
            size) { // loop over all interval (= pieces received from other PEs)
