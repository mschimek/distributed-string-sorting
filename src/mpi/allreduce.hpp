...
static inline std::array<DataType, Length> allreduce_sum(
    ...
    MPI_Allreduce(send_data.data(), result.data(),
        Length * type_mapper<DataType>::factor(), type_mapper<DataType>::type(),
        MPI_SUM, env.communicator());
    return result;
}
...
template <typename DataType>
static inline DataType allreduce_avg(
    DataType& send_data, environment env = environment()) {
    static_assert(std::is_arithmetic<DataType>(),
...
template <typename DataType>
static inline DataType allreduce_ssd(
    DataType& send_data, environment env = environment()) {
    static_assert(std::is_arithmetic<DataType>(),
