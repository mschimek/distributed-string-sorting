...
inline std::vector<DataType> alltoall(
    ...
    MPI_Alltoall(send_data.data(), send_data.size() / env.size(),
        dtm.get_mpi_type(), receive_data.data(), send_data.size() / env.size(),
...
inline std::vector<DataType> alltoallv_small(
    const std::vector<DataType>& send_data,
    ...
    for (size_t i = 1; i < real_send_counts.size(); ++i) {
        send_displacements[i] =
            send_displacements[i - 1] + real_send_counts[i - 1];
    ...
    if constexpr (debug_alltoall) {
        for (int32_t i = 0; i < env.size(); ++i) {
            if (i == env.rank()) {
                ...
                for (const auto sc : real_send_counts) {
                    std::cout << sc << ", ";
                }
                std::cout << std::endl << "receive counts: ";

                for (const auto rc : receive_counts) {
                    std::cout << rc << ", ";
                }
                std::cout << std::endl;
            }
    ...
    data_type_mapper<DataType> dtm;
    MPI_Alltoallv(send_data.data(), real_send_counts.data(),
        send_displacements.data(), dtm.get_mpi_type(), receive_data.data(),
        receive_counts.data(), receive_displacements.data(), dtm.get_mpi_type(),
        env.communicator());
    return receive_data;
}
...
public:
    ...
    static std::vector<DataType> alltoallv(DataType* const send_data,
        ...
        if constexpr (debug_alltoall) {
            for (int32_t i = 0; i < env.size(); ++i) {
                if (i == env.rank()) {
                    std::cout << i << ": send_counts.size() "
                              << send_counts.size() << std::endl;
                    std::cout << i << ": send counts: ";
                    for (const auto sc : real_send_counts) {
                        std::cout << sc << ", ";
                    }
        ...
        data_type_mapper<DataType> dtm;
        MPI_Alltoallv(send_data, real_send_counts.data(),
            send_displacements.data(), dtm.get_mpi_type(), receive_data.data(),
...
public:
    ...
    static std::vector<DataType> alltoallv(DataType* const send_data,
        ...
        const size_t elemToSend =
            ...
            send_counts[env.rank()];
        measuringTool.addRawCommunication(
            elemToSend * sizeof(DataType), "alltoallv_directMessages");
        ...
        for (uint32_t i = 0; i < env.size(); ++i) {
            ...
            MPI_Irecv(receive_data.data() + receive_displacements[source], 1,
                receive_type, source, 44227, env.communicator(),
                &mpi_request[source]);
        }
        // dispatch sends
        for (uint32_t i = 0; i < env.size(); ++i) {
            ...
            MPI_Isend(send_data + send_displacements[target], 1, send_type,
                target, 44227, env.communicator(),
...
public:
    ...
    static std::vector<DataType> alltoallv(DataType* const send_data,
        ...
        size_t local_send_count = std::accumulate(
            send_counts.begin(), send_counts.end(), static_cast<size_t>(0u));

        std::vector<size_t> receive_counts = alltoall(send_counts, env);
        size_t local_receive_count = std::accumulate(receive_counts.begin(),
            receive_counts.end(), static_cast<size_t>(0u));
        ...
        if (global_max < env.mpi_max_int()) {
            ...
            for (uint32_t i = 0; i < env.size(); ++i) {
                ...
                int32_t source = (env.rank() + (env.size() - i)) % env.size();
                auto receive_type =
                    get_big_type<DataType>(receive_counts[source]);
                MPI_Irecv(receive_data.data() + receive_displacements[source],
                    1, receive_type, source, 44227, env.communicator(),
                    &mpi_request[source]);
            }
            // dispatch sends
            for (uint32_t i = 0; i < env.size(); ++i) {
            ...
            MPI_Waitall(
                2 * env.size(), mpi_request.data(), MPI_STATUSES_IGNORE);
            return receive_data;
        }
    }
};
...
static inline std::vector<size_t> computeSendCountsBytes(const StringSet& ss,
    const std::vector<size_t>& intervals, const ByteEncoder& byteEncoder) {

    const auto begin = ss.begin();
    std::vector<size_t> sendCountsBytes(intervals.size(), 0);
    for (size_t interval = 0, offset = 0; interval < intervals.size();
         ++interval) {
        ...
        sendCountsBytes[interval] = byteEncoder.computeNumberOfSendBytes(
            sendCountChars, intervals[interval]);
...
struct AllToAllStringImpl {
    ...
    dss_schimek::StringLcpContainer<StringSet> alltoallv(
        ...
        std::vector<size_t> sendCountsTotal =
            computeSendCountsBytes<StringSet, ByteEncoderPolicy>(
                sendSet, sendCountsString, byteEncoder);

        size_t totalNumberSendBytes = std::accumulate(sendCountsTotal.begin(),
        ...
        for (size_t interval = 0, stringsWritten = 0;
            ...
            StringSet subSet =
                sendSet.sub(begin, begin + sendCountsString[interval]);
            curPos = byteEncoder.write(
                curPos, subSet, container.lcp_array() + stringsWritten);
            stringsWritten += sendCountsString[interval];
        }
        ...
        std::vector<unsigned char> recv =
            AllToAllPolicy::alltoallv(buffer.data(), sendCountsTotal);
        ...
        return StringLcpContainer<StringSet>(
            std::move(rawStrings), std::move(rawLcps));
    }
};
...
struct AllToAllStringImpl<compressLcps, StringSet, AllToAllPolicy,
    ...
    dss_schimek::StringLcpContainer<StringSet> alltoallv(
        ...
        for (size_t interval = 0, stringsWritten = 0;
            ...
            StringSet subSet =
                ss.sub(begin, begin + sendCountsString[interval]);
            size_t numWrittenChars = 0;
            std::tie(curPos, numWrittenChars) =
                byteEncoder.write(curPos, subSet);
...
struct AllToAllStringImpl<compressLcps, StringSet, AllToAllPolicy,
    ...
    dss_schimek::StringLcpContainer<StringSet> alltoallv(
        ...
        measuringTool.stop("all_to_all_strings_mpi");
        measuringTool.add(
            numCharsToSend + send_data.lcps().size(), "bytes_sent");
...
struct RecvDataAllToAll {
    ...
    RecvDataAllToAll(dss_schimek::StringLcpContainer<StringSet>&& container,
        std::vector<size_t>&& recvStringSizes)
...
struct AllToAllStringImpl<compressLcps, StringSet, AllToAllPolicy,
    ...
    dss_schimek::StringLcpContainer<StringSet> alltoallv(
        ...
        for (size_t interval = 0, offset = 0;
             interval < sendCountsString.size(); ++interval) {
            for (size_t j = offset; j < sendCountsString[interval] + offset;
                 ++j) {
                size_t stringLength =
                    sendSet.get_length(sendSet[sendSet.begin() + j]);
                sendCountsChar[interval] += stringLength + 1;
            }
            sendCountsTotal[interval] = byteEncoder.computeNumberOfSendBytes(
                sendCountsChar[interval], sendCountsLcp[interval]);
        ...
        for (size_t interval = 0, offset = 0, stringsWritten = 0;
            ...
            curPos = byteEncoder.write(curPos,
                contiguousStrings.data() + offset, sendCountsChar[interval],
...
dss_schimek::StringLcpContainer<StringSet> getStrings(Iterator requestBegin,
    ...
    offsets.reserve(env.size());
    offsets.push_back(0);
    std::partial_sum(requestSizes.begin(), requestSizes.end() - 1,
        std::back_inserter(offsets));
    ...
    for (size_t curRank = 0; curRank < env.size(); ++curRank) {
        for (size_t i = 0; i < recvRequestSizes[curRank]; ++i) {
            ...
            std::copy_n(
                charsStr, length, std::back_inserter(rawStrings[curRank]));
