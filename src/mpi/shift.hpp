...
static inline DataType shift_left(
    ...
    MPI_Sendrecv(&send_data, 1, dtm.get_mpi_type(), destination,
        0, // chose arbitrary tag
        &receive_data, 1, dtm.get_mpi_type(), source, MPI_ANY_TAG,
        env.communicator(), MPI_STATUS_IGNORE);
    return receive_data;
}

template <typename DataType>
static inline DataType shift_right(
    DataType& send_data, environment env = environment()) {
...
template <typename DataType>
static inline std::vector<DataType> shift_left(
    DataType* send_data, std::size_t count, environment env = environment()) {
    ...
    data_type_mapper<DataType> dtm;
    MPI_Sendrecv(send_data, count, dtm.get_mpi_type(), destination,
        0, // chose arbitrary tag
        receive_data.data(), receive_count, dtm.get_mpi_type(), source,
        MPI_ANY_TAG, env.communicator(), MPI_STATUS_IGNORE);
    return receive_data;
}

template <typename DataType>
static inline std::vector<DataType> shift_right(
    DataType* send_data, std::size_t count, environment env = environment()) {
...
static inline std::vector<dss_schimek::char_type> shift_string_left(
    ...
    MPI_Sendrecv(real_send_data.data(), send_length, dtm.get_mpi_type(),
        destination,
        0, // chose arbitrary tag
        receive_data.data(), receive_length, dtm.get_mpi_type(), source,
        MPI_ANY_TAG, env.communicator(), MPI_STATUS_IGNORE);
    return receive_data;
}
...
static inline ShiftAdress get_adress_right_shift(
...
static inline ShiftAdress get_adress_left_shift(
    environment env = environment()) {
    ShiftAdress adress;
...
static inline ShiftCounts<DataType> get_shift_recv_counts(
    const DataType& send_data, environment env = environment(),
    bool is_empty = false) {
    ...
    DataType recv_data;
    if (is_empty) {
        MPI_Recv(&recv_data, 1, dtm.get_mpi_type(), adress.source, MPI_ANY_TAG,
            env.communicator(), MPI_STATUS_IGNORE);
        MPI_Send(&recv_data, 1, dtm.get_mpi_type(), adress.destination, 42,
            env.communicator());
    }
    else {
        MPI_Request send_request, recv_request;
        MPI_Irecv(&recv_data, 1, dtm.get_mpi_type(), adress.source, MPI_ANY_TAG,
            env.communicator(), &recv_request);
        MPI_Isend(&send_data, 1, dtm.get_mpi_type(), adress.destination, 42,
            env.communicator(), &send_request);
...
static inline std::vector<unsigned char> shift_string(
    ...
    MPI_Irecv(receive_data.data(), recv_length, dtm.get_mpi_type(),
        adress.source, MPI_ANY_TAG, env.communicator(), &requests.data()[1]);
