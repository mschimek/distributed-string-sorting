...
template <typename DataType>
inline std::vector<DataType> allgather(
    DataType& send_data, environment env = environment()) {
...
template <typename DataType>
static inline std::vector<DataType> allgatherv_small(
    std::vector<DataType>& send_data, std::vector<int32_t>& receiving_sizes,
    ...
    for (size_t i = 1; i < receiving_sizes.size(); ++i) {
        receiving_offsets[i] =
            receiving_offsets[i - 1] + receiving_sizes[i - 1];
    }
    ...
    MPI_Allgatherv(send_data.data(), local_size, dtm.get_mpi_type(),
        receiving_data.data(), receiving_sizes.data(), receiving_offsets.data(),
        dtm.get_mpi_type(), env.communicator());
...
static inline std::vector<DataType> allgatherv(
    ...
    if (receiving_sizes.back() + receiving_offsets.back() < env.mpi_max_int()) {
        ...
        for (uint32_t i = 0; i < env.size(); ++i) {
            ...
            MPI_Irecv(receiving_data.data() + receiving_offsets[i], 1,
                receive_type, i, 44227, env.communicator(), &mpi_requests[i]);
        }
