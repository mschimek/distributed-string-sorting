...
struct DuplicateWitness {
    ...
    DuplicateWitness(size_t stringIndex, size_t PEIndex, size_t depth,
        size_t witnessStringIndex, size_t witnessPEIndex)
        : stringIndex(stringIndex), PEIndex(PEIndex), depth(depth),
          witnessStringIndex(witnessStringIndex),
          witnessPEIndex(witnessPEIndex) {}

    friend std::ostream& operator<<(
        std::ostream& stream, const DuplicateWitness& duplicateWitness) {
...
public:
    void printDuplicateWitness(const size_t stringIndex, const size_t PEIndex) {
        for (const DuplicateWitness elem : overallWitnessesPerPE[PEIndex])
            if (elem.stringIndex == stringIndex) {
                ...
                String ownString =
                    ownStringSet[ownStringSet.begin() + stringIndex];
                ...
                String witnessString =
                    witnessStringSet[witnessStringSet.begin() +
                                     elem.witnessStringIndex];
                auto witnessChars =
                    witnessStringSet.get_chars(witnessString, 0);
                std::cout << elem << std::endl;
                std::cout << ownChars << " "
                          << hash(ownChars, elem.depth, filterSize)
                          << std::endl;
                std::cout << witnessChars << " "
                          << hash(witnessChars, elem.depth, filterSize)
                          << std::endl;
            }
    }
    ...
    Tracker(const size_t filterSize)
        : env{}, depth(0), filterSize(filterSize),
          overallWitnessesPerPE(env.size()), curItWitnessesPerPE(env.size()),
          resultsPerPE(env.size()), candidatesPerPE(env.size()),
...
private:
    ...
    void updateForNextRound() {
        for (size_t curRank = 0; curRank < env.size(); ++curRank) {
            ...
            std::copy(curItWitnessesPerPE[curRank].begin(),
        ...
        for (size_t i = 1; i < allHashTriples.size(); ++i) {
            ...
            if (prevHashTriple.hashValue == curHashTriple.hashValue) {
                curItWitnessesPerPE[prevHashTriple.PEIndex].emplace_back(
                    prevHashTriple.stringIndex, prevHashTriple.PEIndex, depth,
                    curHashTriple.stringIndex, curHashTriple.PEIndex);
                duplicate = true;
            }
            else if (duplicate) {
                curItWitnessesPerPE[prevHashTriple.PEIndex].emplace_back(
            ...
            for (size_t i = 0; i < candidatesPerPE[curRank].size(); ++i) {
                ...
                if (depth > length) {
                    ...
                    const size_t curHash =
                        hash(curSet.get_chars(curString, 0), depth, filterSize);
    ...
    void splitCandidates(const std::vector<size_t>& candidatesSizes,
        ...
        for (size_t curRank = 0; curRank < candidatesSizes.size(); ++curRank) {
            ...
            std::copy(curIt, curIt + curCandidatesSize,
                std::back_inserter(candidatesPerPE[curRank]));
            curIt += curCandidatesSize;
        }
    }

    void splitResults(const std::vector<size_t>& resultsSizes,
        ...
        for (size_t curRank = 0; curRank < resultsSizes.size(); ++curRank) {
            ...
            std::copy(curIt, curIt + curResultsSize,
                std::back_inserter(resultsPerPE[curRank]));
    ...
    void splitContainer(const std::vector<size_t>& stringSetSizes) {
        ...
        for (size_t curRank = 0; curRank < stringSetSizes.size(); ++curRank) {
            StringSet newStringSet(begin + curStringSetBound,
