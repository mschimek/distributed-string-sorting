...
struct RadixStep_CE0_lcp {
  tlx::sort_strings_detail::StringShadowPtr<StringSet> strptr;
    ...
    RadixStep_CE0_lcp(const tlx::sort_strings_detail::StringShadowPtr<StringSet>& in_strptr, size_t depth)
        : strptr(in_strptr) {
        ...
        {
          if (strptr.flipped())
          strptr.shadow().set_lcp((bkt_index[j] - bkt_index[0]) + bkt_size[j], depth);
          else 
          strptr.active().set_lcp((bkt_index[j] - bkt_index[0]) + bkt_size[j], depth);
          ++j;
        }
        // distribute
        for (Iterator i = ss.begin(); i != ss.end(); ++i)
        ...
        const auto& correct_set = strptr.flip(0, pos).copy_back();
        for(size_t i = 1; i < pos; ++i)
          correct_set.set_lcp(i, depth);
    }
};
...
radixsort_CE0(const StringSet& ss, size_t depth, size_t memory) {
    ...
    typedef RadixStep_CE0_lcp<StringSet> RadixStep;
...
struct RadixStep_CE2_lcp {
  ...
  typedef typename StringSet::Iterator Iterator;
    ...
    RadixStep_CE2_lcp(const tlx::sort_strings_detail::StringShadowPtr<StringSet>& in_strptr, size_t depth,
...
template <typename StringSet>
static inline void
radixsort_CE2(const tlx::sort_strings_detail::StringShadowPtr<StringSet>& strptr,
              uint8_t* charcache, size_t depth, size_t memory) {

    typedef RadixStep_CE2_lcp<StringSet> RadixStep;
...
radixsort_CE2(const StringSet& ss, size_t depth, size_t memory) {
    ...
    if (memory != 0 && memory < memory_use + memory_slack + 1)
        return mysorter::radixsort_CI3(ss, depth, memory);
    ...
    mysorter::radixsort_CE2<StringSet>(
