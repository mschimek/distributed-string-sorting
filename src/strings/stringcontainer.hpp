...
public:
    ...
    std::vector<String> init_strings(std::vector<Char>& raw_strings,
        const std::vector<uint64_t>& stringIndices) {
        ...
        for (size_t i = 0; i < raw_strings.size(); ++i) {
            strings.emplace_back(
                raw_strings.data() + i, i, stringIndices[curString]);
            while (raw_strings[i] != 0)
                ++i;
            strings.back().length = i - strings.back().length;
            ++curString;
...
public:
    ...
    void extendPrefix(StringSet ss, const std::vector<size_t>& lcps) {
        ...
        size_t curPos = 0u;
        std::copy(startCurString, startCurString + stringLength,
        ...
        for (size_t i = 1; i < ss.size(); ++i) {
            ...
            std::copy(curPrefix.begin(), curPrefix.end(),
                extendedRawStrings.begin() + curPos);
